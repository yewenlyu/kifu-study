...
describe("board panning", () => {
  it("pans only an oversized board with a moved middle-button drag", () => {
  ...
  it("ignores left-drag and right-drag on an oversized board", () => {
    const { container } = renderApp();
    const stage = container.querySelector<HTMLDivElement>(".board-stage")!;
    makeStageScrollable(stage);
    stage.scrollLeft = 100;
  ...
  it("leaves a fitting board and stationary middle-click unchanged", () => {
    const { container } = renderApp();
    const stage = container.querySelector<HTMLDivElement>(".board-stage")!;
  ...
  it("ends panning when pointer capture is lost", () => {
    ...
    fireEvent.pointerDown(stage, { button: 1, pointerId: 4 });
    expect(stage.classList.contains("is-panning")).toBe(true);
