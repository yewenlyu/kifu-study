...
| Board | Render contrast-aware triangle and circle marks on any intersection, with opaque circles on empty points | `domain/go/model.test.ts`; `diagram-marks.test.tsx` |
...
| Navigation | Fill the visible desktop workspace with the board frame while keeping the compact status bar attached without trailing frame space | `status-bar.spec.ts` |
